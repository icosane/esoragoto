...
def ru(request):
    ...
    html_fig2 = mpld3.fig_to_html(fig2, template_type='general')
    plt.close(fig2)
    return render(request, 'ru.html', {'div_figure1': html_fig1, 'div_figure2': html_fig2})

def graph_n1(request):
    N_ = int(request.POST['N'])
    K = int(request.POST['K'])
    interval_t = 100  # пространство i
    t_ = float(request.POST['t'])
...
def graph_n1ru(request):
    ...
    return render(request, 'graph_n1ru.html', {'div_figure': html_fig})
...
def graph_n2ru(request):
    ...
    plt.close(fig)
    return render(request, 'graph_n2ru.html', {'div_figure': html_fig})